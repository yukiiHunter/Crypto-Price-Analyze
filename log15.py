...
def calculate_percentage_change(symbol, interval='15m'):
...
def calculate_percentage_change1(symbol, interval='15m'):
    candles = client.get_klines(symbol=symbol, interval=interval)
...
def main():
    ...
    selection = st.sidebar.radio("Go to", ["Log 15 Minutes"], captions = ["[Log 1 minutes](https://cryptoanalyzelog.streamlit.app/) | [Log 5 minutes](https://cryptoanalyzelog5.streamlit.app/)"])

    if 'selected_symbols' not in st.session_state:
        st.session_state.selected_symbols = []

    if selection == "Log 15 Minutes":
